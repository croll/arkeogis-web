 ...
 *insert into group__permission values (1, 7, now(), now());chj
...
(function () {
	...
	ArkeoGIS.controller('CharacEditorListCtrl', ['$scope', '$q', '$http', 'arkeoLang', 'login', function ($scope, $q, $http, arkeoLang, Login) {
		...
		if (!Login.requirePermission('user can edit some charac', 'arkeogis.langeditor'))
            return;
	...
	/*
	 * CharacEditorMapCtrl Charac Editor Map Controller (choix de l'emprise)
	 */

	ArkeoGIS.controller('CharacEditorMapCtrl', ['$scope', 'mapService', 'leafletData', '$rootScope', '$timeout', function ($scope, mapService, leafletData, $rootScope, $timeout) {

		var resize = function() {
			$scope.mapHeight = ($(window).height() - $("#arkeo-main-toolbar").height() - $("#arkeo-characeditor-toolbar").height())+"px";
		};
	...
	}]); // controller CharacEditorMapCtrl
